...
# ── Duong dan ──────────────────────────────────────────────────────────────────
SCRIPT_DIR = Path(__file__).parent           # GetPgsImageApp/
PARENT_DIR = SCRIPT_DIR.parent               # 3.Tools/  (chua package 'tool')
PY_FILE    = SCRIPT_DIR / "RunGetPgsImage.py"
APP_NAME   = "GetPgsImage"
...
# ── Lenh PyInstaller (toi gian — chi can tkinter + stdlib) ────────────────────
_PYINSTALLER_FULL = [
    ...
    "--hidden-import", "tool.features.collection.pgs_image",
    "--hidden-import", "tool.features.collection.tab_pgs_image",
    # tkinterdnd2 (optional — khong bat buoc)
    "--hidden-import", "tkinterdnd2",
    "--collect-all",   "tkinterdnd2",
    # PIL / Pillow -- can cho pHash dedup
    "--hidden-import", "PIL",
    "--hidden-import", "PIL.Image",
    # ── Loai bo tat ca framework nang khong can cho PGS ──────────────────────
    "--exclude-module", "cv2",
    "--exclude-module", "numpy",
    "--exclude-module", "matplotlib",
    "--exclude-module", "requests",
    "--exclude-module", "urllib3",
    "--exclude-module", "minio",
...
class BuildApp(Tk):
    ...
    def _build_ui(self):
        ...
        Label(hdr, text="  KZTEK  -  Build Get PGS Image",
              bg=ACCENT2, fg="white",
            ...
            for raw_line in self._proc.stdout:
                ...
                for keyword, pct in _STEPS:
                    if keyword in line.lower() and pct > self._progress:
                        ...
                        break
            self._proc.wait()
            self.after(0, self._on_done, self._proc.returncode)
