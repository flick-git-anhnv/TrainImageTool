"""
KZTEK Fine-tune — 50 epochs, tiep tuc tu best.pt
Dataset: tai su dung K:/kztek_split_test10ep (da co san)
Fine-tune: lr0=0.001 (giam 10x), close_mosaic=5, cos_lr=True
"""
...
BEST_PT   = 'E:/KZTEK/AI/test-model/model-by-claude11/kztek_train/weights/best.pt'
YAML_PATH = 'K:/kztek_split_test10ep/data.yaml'
PROJECT   = 'E:/KZTEK/AI/test-model/model-by-claude11'
RUN_NAME  = 'kztek_finetune'
EPOCHS    = 50
...
if __name__ == '__main__':
    ...
    print('=' * 60)
    print('KZTEK Fine-tune')
    print(f'  Base model : {BEST_PT}')
    print(f'  Dataset    : {YAML_PATH}')
    print(f'  Epochs     : {EPOCHS}')
    print(f'  LR0 / LRF  : 0.001 / 0.001  (fine-tune, giam 10x)')
    print(f'  cos_lr     : True')
    ...
    results = model.train(
        ...
        exist_ok=True,
        optimizer='AdamW',
        lr0=0.001,          # fine-tune: giam 10x so voi train moi
        lrf=0.001,
        close_mosaic=5,     # 10% cua 50 epochs
        cache=False,
        workers=4,
        cos_lr=True,
        weight_decay=0.0005,
        patience=15,        # early stop neu 15 epoch khong cai thien
    )
